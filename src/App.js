...
const App = () => {
  ...
  // Helps to switch to QuestionList screen when pressing start button
  const handleGameStart = () => setGameStart(prevGameStart => !prevGameStart);

  // Determines when to show an error message if api data does not match user specifications
  const handleQuestionsNotFound = boolean => setShowQuestionsError(boolean);
